  ...
  data: {      
   ...
   methods: {
    ...
    attack_init: function (id) {    
      $('.adds_fake').trigger('click');      
      vm.id_attacker = id; 
      if (vm.nitro == 0) {
        $('.nitro_btn').attr('disabled', true);
        $('.nitro_btn').addClass('disabled');        
      } else {
        $('.nitro_btn').attr('disabled', false);
        $('.nitro_btn').removeClass('disabled');
      };

      if (vm.energy == 0) {
        $('.enrg_btn').attr('disabled', true);
        $('.enrg_btn').addClass('disabled');
      } else {
        $('.enrg_btn').attr('disabled', false);
        $('.enrg_btn').removeClass('disabled');
      };

      $('#adds .trace .pick:first_child').addClass('picked');
    },
    go_race: function() {
      var to = dappAddress;
      var value = 0;
      ...
      var road = $('#adds .trace .picked').attr('trace-id');     
      var defender = vm.id_defender;
      var items = $('#adds .items_adds .picked').attr('item-id');
      var args = [];
      vm.road = road;
      vm.items = items;
      args.push(attacker);
    ...
    user_page: function() {      
      ...
      var to = dappAddress;
      var value = 0;
      var callFunction = 'getFullMyInfo';                
      var args = [];      
      var callArgs = JSON.stringify(args);      
      nebPay.simulateCall(to, value, callFunction, callArgs, { 
        listener: cbUserPage
...
// маркет темплейт
    // <div class="owner">owner: <span v-on:click="vm.user_page(owner)" class="value">{{owner}}</span></div>
...
// результат гонки темплейт
    ...
    template: `<div class="car">
                  ...
                  <div class="owner">owner: <span class="value">{{owner}}</span></div>
                  <span class="time_race">{{time}}</span>                  
...
// обработчик гонки   
  ...
  function cbTransactionRace(resp) {
   ...
   function cbRaceRender(resp) {        
    ...
    attacker.time = 'Result time: ' + race_arr.timeA + ' min';
    defender.time = 'Result time: ' + race_arr.timeD + ' min';

    if (attacker.time == 9999) {
      attacker.time = 'The car broke down';
    } else {
      defender.time = 'The car broke down';
    }
...
// обработчик пика
  $('#adds .trace .pick').click(function(){  
    $('#adds .trace .pick').removeClass('picked');
    $('#adds .trace .pick').html('Pick');        
    $(this).addClass('picked');
    $(this).html('Picked');
  });

  $('#adds .items_adds .pick').click(function(){  
    if ($(this).hasClass('picked')) {     
        $(this).removeClass('picked');
        $(this).html('Pick');
        return false;
    };
    $('#adds .items_adds .pick').removeClass('picked');
    $('#adds .items_adds .pick').html('Pick');            
    $(this).addClass('picked');
    $(this).html('Picked');
  })
// обработчик пика
...
// гет юзер хистори  
  function cbUserPage(resp) {    
    ...
    var history_player = resp.history;    
    $('.wrapp_history').html('');
    history_player.reverse();
    $.each(history_player, function(index,value){
      var attacker_name = fix_name(history_player[index].atacker.name);
      var defender_name = fix_name(history_player[index].defender.name);
      var result = history_player[index].res;           
      if (result == "Win") {
        $('.wrapp_history').append(`
                                  <li fight-id="` + index + `">
                                    <span class="opponent"><img src="img/` + attacker_name + `.png" alt=""></span>
                                    <span class="opponent"><img src="img/` + defender_name + `.png" alt=""></span>
                                    <span class="result win">` + result + `</span>
                                    <span class="info"><a href="#"><img src="img/more_info.png" alt="" class="more_info"></a></span>
                                  </li>
                                `);
      } else {
         $('.wrapp_history').append(`
                                  <li fight-id="` + index + `">
                                    <span class="opponent"><img src="img/` + attacker_name + `.png" alt=""></span>
                                    <span class="opponent"><img src="img/` + defender_name + `.png" alt=""></span>
                                    <span class="result loose">` + result + `</span>
                                    <span class="info"><a href="#"><img src="img/more_info.png" alt="" class="more_info"></a></span>
                                  </li>
                                `); 
      };      
      $.each(history_player, function(index,value){
        vm.player_history.push(history_player[index]);
      });        
    });

    // страница гонки из истории
    $('.more_info').click(function(){      
      ...
      vm.fight_id = $(this).parent().parent().parent().attr('fight-id');

      // n1e3z1jSPckHmnwwTtD3AMHKh6peU3KQeW6
      // geMyAddress            
      var NebPay1 = require("nebpay");   
      var nebPay1 = new NebPay1();
      var dappAddress1 = "n1e3z1jSPckHmnwwTtD3AMHKh6peU3KQeW6";

      var to = dappAddress1;
      var value = 0;
      var callFunction = 'geMyAddress';
      var callArgs = "[]";        
      nebPay1.simulateCall(to, value, callFunction, callArgs, { 
        listener: cbFixRace
      });     

    });

    function cbFixRace(resp) {  
      var owner = JSON.parse(resp.result);       
      var race_arr = vm.player_history[vm.fight_id];      
      var attacker = race_arr.atacker;
      var defender = race_arr.defender;    
      var result = race_arr.res;
      console.log('attacker.name ' + attacker.name);
